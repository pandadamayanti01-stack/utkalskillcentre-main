...
const GunduluSishuVatika = ({ skipInitialGreeting = false, userClass, onBack, isPremium = false, onUpgrade, user }: { skipInitialGreeting?: boolean; userClass?: string; onBack?: () => void; isPremium?: boolean; onUpgrade?: () => void; user?: any }) => {
  ...
  const [inputLanguage, setInputLanguage] = useState('or-IN'); // Default to Odia ASR for Sishu Vatika
    ...
    try {
      ...
      const systemInstruction = `
        ...
        Teacher Behavior Rules:
          ...
          5. Do NOT mention any technical English terms, English translations, or bilingual comparisons. Speak and explain using only standard, extremely simple Odia terms appropriate for a 3-5 year old child.
        ...
        Language Policy: STRICT 100% ODIA ONLY. You MUST write your entire response ONLY in complete Odia. Do NOT include any English words, English translations, or bilingual comparisons. The text, greetings, and explanation must be 100% Odia.
        ...
        Constraint: Keep your response extremely brief, simple, and conversational (maximum 1-2 short sentences, under 30 words) so that the student isn't overwhelmed and the voice synthesis is nearly instantaneous (like a real WhatsApp voice call!). Never give long list-style lectures. Since the child is in Anganwadi, keep the language extremely gentle, simple, sweet, and strictly in Odia.
        ...
        {activeChapter && (
          <div className="textbook-visual-card animate-fade-in">
            ...
            <div className="card-notes-preview">
              <p className="notes-heading">ପାଠ୍ୟକ୍ରମ ବିବରଣୀ:</p>
      ...
      <div className="call-bottom-hud-panel single-stop-hud" style={{ display: 'flex', gap: '24px', alignItems: 'center' }}>
        <button 
          ...
          onClick={() => {
            ...
            stopCurrentAudio();
            setIsSpeaking(false);
            setStatus(nextState ? "ଉନ୍ନତ ସ୍ଵର ସକ୍ରିୟ ହେଲା ✨" : "ସାଧାରଣ ସ୍ଵର ସକ୍ରିୟ ହେଲା 📱");
        ...
        >
          ...
          <span className="hud-btn-label" style={{ fontSize: '0.62rem', whiteSpace: 'nowrap', marginTop: '1px' }}>
            {usePremiumVoice ? "ଉନ୍ନତ ସ୍ଵର" : "ସାଧାରଣ ସ୍ଵର"}
          </span>
        </button>
...
export default GunduluSishuVatika;
