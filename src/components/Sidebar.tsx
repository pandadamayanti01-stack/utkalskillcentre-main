...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const allMenuItems = [
    ...
    { id: 'study_buddy', icon: Lucide.MessageSquare, label: user?.role === 'teacher' ? (language === 'en' ? 'Gundulu Instructor' : 'ଗୁଣ୍ଡୁଲୁ ଇନଷ୍ଟ୍ରକ୍ଟର') : t.studyBuddy },
    { id: 'gundulu', icon: Lucide.Mic, label: language === 'en' ? 'Gundulu AI Tutor' : 'ଗୁଣ୍ଡୁଲୁ AI ଟ୍ୟୁଟର' },
    { id: 'digital_library', icon: Lucide.Library, label: language === 'en' ? 'Digital Library' : 'ଡିଜିଟାଲ୍ ଲାଇବ୍ରେରୀ' },
    { id: '3d_study', icon: Lucide.Box, label: language === 'en' ? 'Gundulu 3D Lab' : 'ଗୁଣ୍ଡୁଲୁ ୩D ଲ୍ୟାବ୍' },
    ...
    { 
      ...
      label: user?.role === 'teacher'
        ? (language === 'en' ? 'Sikhyaka Sathi' : 'ଶିକ୍ଷକ ସାଥୀ')
        : (language === 'en' ? 'Mo Sanga' : 'ମୋ ସାଙ୍ଗ')
    },
    ...
    { id: 'daily_mcqs', icon: Lucide.ListChecks, label: language === 'en' ? 'Gundulu Daily Challenge' : 'ଗୁଣ୍ଡୁଲୁ ଦୈନିକ ଚ୍ୟାଲେଞ୍ଜ' },
    { id: 'game_zone', icon: Lucide.Gamepad2, label: language === 'en' ? 'Gundulu Game Zone' : 'ଗୁଣ୍ଡୁଲୁ ଗେମ୍ ଜୋନ୍' },
