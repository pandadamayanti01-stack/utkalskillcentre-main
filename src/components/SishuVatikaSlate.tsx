...
export const SishuVatikaSlate: React.FC<SishuVatikaSlateProps> = ({
  language,
  onClose,
  isPremium = false,
  onUpgrade,
  ...
  const [selectedLang, setSelectedLang] = useState<'en' | 'or'>('or');
  ...
  const loadTemplate = (type: 'vowel' | 'consonant' | 'number1' | 'number2' | 'circle') => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    const ctx = canvas.getContext('2d');
    if (!ctx) return;
    ...
    ctx.strokeStyle = 'rgba(254, 248, 236, 0.25)';
    ctx.fillStyle = 'rgba(254, 248, 236, 0.25)';
    ctx.lineWidth = 4;
    ctx.lineCap = 'round';
    ctx.lineJoin = 'round';
    ...
    if (type === 'vowel') {
      ...
      ctx.setLineDash([5, 8]);
      ctx.strokeText("ଅ", width / 2, height / 2);
      ctx.setLineDash([]);
    } else if (type === 'consonant') {
      ctx.font = 'bold 160px "Arial", sans-serif';
      ctx.setLineDash([5, 8]);
      ctx.strokeText("କ", width / 2, height / 2);
      ctx.setLineDash([]);
    } else if (type === 'number1') {
      ctx.font = 'bold 160px "Arial", sans-serif';
      ctx.setLineDash([5, 8]);
      ctx.strokeText("୧", width / 2, height / 2);
      ctx.setLineDash([]);
    } else if (type === 'number2') {
      ctx.font = 'bold 160px "Arial", sans-serif';
      ctx.setLineDash([5, 8]);
      ctx.strokeText("୨", width / 2, height / 2);
      ctx.setLineDash([]);
    } else if (type === 'circle') {
      ctx.beginPath();
  ...
  const speakText = async () => {
    ...
    if (!usePremium && 'speechSynthesis' in window) {
      ...
      utterance.pitch = 1.75; // cute 1-2 year old baby girl high pitch
      utterance.rate = 0.72;  // slower cute toddler rate
      ...
      try {
        const response = await fetch('/api/tts/anganwadi', {
          method: 'POST',
  ...
  const solveProblem = async () => {
    ...
    try {
      ...
      // Sishu Vatika/Anganwadi Vision Prompt
      const systemInstruction = 
        ...
        "Analyze the child's drawing or tracing on the chalkboard. " +
        "1. Identify the letter, number, shape, or picture drawn. Look for preschool-level things: Odia letters (like 'ଅ', 'କ'), Odia numbers (like '୧', '୨'), simple shapes (like circle/ଗୋଲ, triangle, square), or simple drawings of animals/objects. " +
        ...
        "3. CRITICAL: Speak ONLY in Odia. Do NOT use English words, translations in brackets, or bilingual text. Everything must be in Odia. " +
        "4. Keep the explanation very short and simple (1-3 lines max) so a toddler can understand when it is read aloud. " +
        "5. Give a playful little instruction or challenge in Odia (e.g., 'ଏବେ ଏକ ଗୋଲ ଆଙ୍କିବାକୁ ଚେଷ୍ଟା କର!' or 'କହିଲ ଦେଖି, କ ପରେ କଣ ଆସେ?'). " +
        "6. Do NOT write any English sentences or letters, nor explain anything in English.";

      const promptText = "ଏହି ଚିତ୍ର କିମ୍ବା ଲେଖାକୁ ଚିହ୍ନଟ କରି ସରଳ ଓ ଅତି ସୁନ୍ଦର ଓଡ଼ିଆ ଭାଷାରେ ପିଲାଟିକୁ ବୁଝାନ୍ତୁ ଏବଂ ତାକୁ ଉତ୍ସାହିତ କରନ୍ତୁ।";
        ...
        <div className="w-full h-full p-6 flex flex-col gap-4 bg-slate-950/40 relative min-h-0">
          ...
          <div className="flex items-center justify-between border-b border-slate-800/60 pb-3.5 gap-4 flex-wrap md:flex-nowrap">
            ...
            <div className="flex items-center gap-3 ml-auto">
              <div className="flex items-center gap-2 bg-slate-950/80 p-1.5 rounded-full border border-white/5 shadow-inner">
                <span className="text-[8px] text-slate-400 font-black uppercase tracking-wider pl-1.5">ରଙ୍ଗ:</span>
                <button
          ...
          <div className="flex flex-wrap items-center gap-2 bg-slate-950/45 p-3 rounded-2xl border border-white/5 relative overflow-hidden shrink-0">
            ...
            <span className="text-[10px] text-amber-400 font-black uppercase tracking-wider flex items-center gap-1.5 z-10 relative">
              ...
              ଲେଖା ଓ ଚିତ୍ର ଅଭ୍ୟାସ (Tracing Templates):
            </span>
            <div className="flex flex-wrap gap-1.5 z-10 relative">
              <button
                type="button"
                onClick={() => loadTemplate('vowel')}
              ...
              >
                <span>✍️ ସ୍ଵରବର୍ଣ୍ଣ 'ଅ'</span>
              </button>
              <button
                type="button"
                onClick={() => loadTemplate('consonant')}
                className="px-3 py-1.5 rounded-xl bg-slate-900/90 border border-white/10 hover:border-amber-500/50 hover:bg-slate-800/80 text-white text-[10px] font-black uppercase tracking-wider transition-all cursor-pointer hover:scale-[1.02] active:scale-95 flex items-center gap-1"
              >
                <span>✍️ ବ୍ୟଞ୍ଜନବର୍ଣ୍ଣ 'କ'</span>
              </button>
              <button
                type="button"
                onClick={() => loadTemplate('number1')}
                className="px-3 py-1.5 rounded-xl bg-slate-900/90 border border-white/10 hover:border-amber-500/50 hover:bg-slate-800/80 text-white text-[10px] font-black uppercase tracking-wider transition-all cursor-pointer hover:scale-[1.02] active:scale-95 flex items-center gap-1"
              >
                <span>🔢 ସଂଖ୍ୟା '୧'</span>
              </button>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 bg-slate-900/60 p-3 rounded-2xl border border-white/5">
            ...
            {/* Brush Width Slider */}
            <div className="flex items-center gap-2">
              <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">ଖଡ଼ିର ମୋଟେଇ:</span>
              <input
                type="range"
                min="2"
                max="12"
              ...
              {!loading && explanation && (
                <div className="flex items-center gap-3 border-t border-slate-800 pt-3 flex-shrink-0">
                  ...
                  >
                    ...
                    <span>{speaking ? 'ଅଟକାନ୍ତୁ' : 'ଉତ୍ତର ଶୁଣନ୍ତୁ'}</span>
                  </button>
